...
```(defstruct (sys:backtrace-frame (:named) (:type vector))
      type return-address raw-name function-name print-name frame-size frame-offset function-start-address function-end-address base-pointer arguments closure function-description)```
...
`sys:ihs-source-information (&optional (index sys:*ihs-current*))` - Return the source information for the frame at index.  (values filename function-name source line column function-start-line discriminator)
`sys:ihs-return-address (&optional (index sys:*ihs-current*))` - 
